...
def change_a():  # Function to switch the turn
    global a
    for i in ['O', 'X']:
        if not i == a:
            a = i
...
def check():  # checks for win or draw
    for i in range(3):
        if b[i][0]['text'] == b[i][1]['text'] == b[i][2]['text'] == a \
            or b[0][i]['text'] == b[1][i]['text'] == b[2][i]['text'] \
            == a:
    ...
    if b[0][0]['text'] == b[1][1]['text'] == b[2][2]['text'] == a \
        ...
        == DISABLED:
        messagebox.showinfo('The game is a tie')
        reset()


def click(row, col): 
    b[row][col].config(text=a, state=DISABLED,
                       disabledforeground=colour[a])
    check()
    change_a()
